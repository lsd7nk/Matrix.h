...
The `matrix_type` field defines the matrix type 
...
The basic actions with matrices are implemented: _create_matrix_(creation), _remove_matrix_(cleaning and destruction), _eq_matrix_(comparison), _sum_matrix_(addition), _sub_matrix_(subtraction), _mult_matrix_(multiplication), _mult_number_(multiplication by number), _transpose_(transpose), _determinant_(calculation of determinant), _calc_complements_(calculation of matrix of algebraic complements), _inverse_matrix_(finding inverse of the matrix).
...
- The matrix is implemented in the form of the structure described [above](#matrix-structure-in-c-language)
